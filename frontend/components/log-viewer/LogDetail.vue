  ...
  >
    <v-card height="800px" class="card-log" :style="logLevelColor(log.level)">
      ...
      <v-card-text style="height: 300px;">
        ...
        <div class="ml-4" v-for="(prop, i) in Object.keys(log.tags)" :key="'tag' + i">
          <p class="default-prop"><b>{{ prop }}:</b> {{ log.tags[prop] || 'N/A' }}</p>
        </div>

        <h3 class="mt-4 mb-2">Custom Properties</h3>
        <div class="ml-4" v-for="(prop, i) in Object.keys(log.businessProperties)" :key="'prop' + i">
          <p class="default-prop"><b>{{ prop }}:</b> {{ log.businessProperties[prop] || 'N/A' }}
            <v-btn
              depressed
              x-small
              color="primary"
